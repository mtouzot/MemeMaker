...
def overlayImage(bg, fg, fgMask, coords):
    """Overlays the foreground image fg on top of the
    background image bg at location coord, allowing for alpha
    transparency via the foreground mask fgMask"""
    # grab the foreground spatial dimensions (width and height),
    # the unpack the coordinates tupe (i.e. where in the image
    ...
    # width and height as our input, but will contain only
    # our foregrounfd mask
    ...
    output = alphaBlend(overlay, bg, alpha)
...
def alphaBlend(fg, bg, alpha):
    ...
    # unsigned 8-bit intergersto floats, making sure to scale the
    ...
    # add the foregrounf and background to obtain the final output
    # image
...
def createGIF(inputPath, outputPath, delay, finalDelay, loop):
    # grab all image path in the input directory
...
for (i, y) in enumerate(steps):
    ...
    output = overlayImage(image, sg, sgMask,
    ...
    if i == len(steps) - 1:
        ...
        oX = int(W * 0.1)
        oY = int(H * 0.8)
        output = overlayImage(output, dwi, dwiMask, (oX, oY))
...
createGIF(config["temp_dir"], args["output"], config["delay"],
